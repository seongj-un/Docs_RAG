...
0012 가 이 마이그레이션을 예고해 두었다: "W6 가 서버 생성 MCP 툴을 만들면
...
같은 값을 남기면 그 비교는 **오늘 이후 영영 복원 불가능**해진다. 0012 가
``llm_model IS NULL`` 이라는 파생 표식을 사실로 바꾼 것과 같은 이유이고, 같은
사고를 W6 에서 한 번 더 막는 것이다.

ENUM 이 아니라 CHECK 인 것이 여기서 값을 한다. 0012 가 그렇게 고른 이유가
...
**이 파일은 한때 0015 였다.** main 병합이 같은 0010 번호를 들고 오는 바람에
M7 쪽 사슬을 통째로 main 의 0010 뒤로 다시 엮었고 — 이유는 0013 의 주석에
있다 — 그 과정에서 결번이던 0012·0014 가 메워지면서 이 마이그레이션이
0014 가 됐다. 바뀐 것은 번호뿐이고, 이 파일은 여전히 0013 바로 뒤에 온다.
"""
