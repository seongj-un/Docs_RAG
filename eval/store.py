...
async def save(session: AsyncSession, report: RunReport) -> uuid.UUID:
    ...
    run = EvalRun(
        ...
        heading_prefix=report.heading_prefix,
        reindex_seconds=report.reindex_seconds,
...
def _to_report(run: EvalRun) -> RunReport:
    return RunReport(
        ...
        heading_prefix=run.heading_prefix,
        reindex_seconds=run.reindex_seconds,
        index_chunks=run.index_chunks,
