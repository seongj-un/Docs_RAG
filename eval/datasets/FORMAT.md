...
- **precision@k** — 상위 k개 청크 중 정답 스팬을 덮는 것의 비율. 분모는 실제로
  돌려준 개수다(코퍼스가 작아 k개를 못 채운 실행을 벌주지 않기 위해). 천장이
  1.0 이 아닌 것이 정상이다 — 정답 스팬이 하나인 질문의 P@10 은 잘해야 0.1~0.2
  다. 절대값이 아니라 **같은 k 에서 구성 간 비교**로 읽는다. recall 옆에 두는
  이유는 W5 의 함정 때문이다: recall 만 올리고 precision 을 버리면 컨텍스트가
  늘어 비용과 지연이 나빠지는데, precision 을 내지 않으면 그 함정에 빠졌는지를
  확인할 방법 자체가 없다.
...
| `spec_golden.jsonl` | 44 | spec_api · spec_errors · spec_ops | **M7 W5 검색 파이프라인 A/B 용.** 헤딩 계층·표·정확일치 토큰·중복 표현을 갖춘 명세형 코퍼스 |

셋 다 **합성 코퍼스에서 기계 변환한 것**이다(`build_synthetic.py`,
`build_spec.py`). 하네스가 제대로 도는지를 검증하기 위한 픽스처이지, M7 의
측정 대상이 아니다. 진짜 측정은 업무 문서 골든셋 60~80문항으로 하고, 그건
private repo 에 있다.

**알려진 한계 — 유형 분포가 W1 목표와 다르다.**

| 유형 | W1 목표 | `synthetic_golden` | `spec_golden` |
| --- | --- | --- | --- |
| `single_fact` | 30% | 61% | 36% |
| `multi_doc` | 25% | 11% | 23% |
| `rule_exception` | 20% | 19% | 18% |
| `no_answer` | 15% | 8% | 14% |
| `freshness` | 10% | **0%** | 9% |

`synthetic_golden` 의 `freshness` 가 0인 것은 채우다 만 것이 아니라 **채울 수
없어서**다. 그 합성 코퍼스에는 개정 이력도 버전도 없어서 "오래된 값을 물고
오는가"를 잴 대상 자체가 없었다. 없는 것을 있는 척 채우면 그 문항들이
freshness 를 측정한다고 거짓으로 보고하게 된다. 또 `synthetic_golden` 의
`multi_doc` 은 엄밀히는 **한 문서 안의 두 조항**을 합치는 문항이다(M4 의
`multi_hop`). L1 에서 요구하는 계산은 동일하지만(다중 스팬 macro
recall·nDCG), 문서 간 혼동 자체는 측정되지 않는다.

**`spec_golden` 이 그 두 칸을 메운다.** `spec_ops` 의 변경 이력 쪽이 v1 의 옛
값을 그대로 싣고 있어서(현행 값은 엔드포인트 쪽에 있다) "오래된 값을 물고
오는가"가 처음으로 잴 대상을 갖는다. `multi_doc` 도 진짜로 문서를 건넌다 —
`spec_api` 는 오류가 *언제* 나는지만 적고 *무엇을 해야 하는지*는
`spec_errors` 에만 있어서, 두 문서를 모두 가져와야 답이 된다.

다만 이것도 **합성**이다. 개정 이력이 한 쪽에 정리돼 있는 것은 실제 업무
문서보다 친절하고, 실제 문서에서는 옛 값이 본문 여기저기에 흩어져 있다.
`spec_golden` 의 freshness 점수는 "이 정도로 정리된 문서에서는 되더라"까지만
말한다.
