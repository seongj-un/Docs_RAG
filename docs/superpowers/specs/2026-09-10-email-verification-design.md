...
토큰을 가져가는 것은 **조건부 UPDATE 한 번**이어야 한다. 읽어서 판단하고
다시 쓰는 방식(check-then-act)은 두 가지를 깬다 — 같은 링크를 두 탭에서
열면 둘 다 "아직 안 썼다"를 보고 둘 다 성공하고, 재발송이 그 행을 지우는
순간에 걸리면 0행을 갱신해 `StaleDataError`가 밖으로 튄다. `WHERE
consumed_at IS NULL AND expires_at > now ... RETURNING`으로 합치면 Postgres
의 행 잠금이 동시 호출을 직렬화해 정확히 하나만 가져가고, 지워진 행은
그냥 0행 매치로 흡수된다.
...
셈 자체는 `usage_events`를 그대로 쓴다. 새 테이블이 필요 없는 이유는 그
테이블이 이미 원하는 성질을 갖고 있기 때문이다: `UsageEvent`의 외래키는
`documents`가 아니라 `users`를 향하므로 **문서를 지워도 행은 남는다.**
올리고 지워서 한도를 되돌리는 우회가 구조적으로 막혀 있다. `documents`를
세었다면 그 우회가 열린다.

다만 기존 `kind`를 그대로 쓸 수는 없다. `"ingest"` 행은 백그라운드 인덱싱이
**성공한 뒤에야** 생긴다(`services/ingest.py:82`). 그걸 세면 구멍이 둘이다 —
색인이 끝나기 전에 업로드를 연달아 던지면 카운터가 0인 채로 전부 통과하고,
깨진 PDF나 임베딩 서버 장애로 실패한 업로드는 아예 세어지지 않는다.

그래서 업로드를 **수락하는 그 요청 안에서** `kind="upload"` 이벤트를 남기고
게이트는 그것을 센다. `"ingest"`는 지금처럼 색인 성공 시에만 기록되고 쪽수
쿼터가 계속 쓴다. 둘을 나눈 덕에 "수락 5 / 색인 3 = 실패 2"가 그냥 읽히는
것은 덤이다. 비용 리포트(`scripts/cost_report.py`)는 `"upload"`를 집계에서
빼야 한다 — 토큰을 쓰지 않는 사건이라 "요청 N건"에 들어가면 캐시 적중률이
낮아 보인다.
...
### 알려진 한계: 동시 요청으로 한도를 넘길 수 있다

게이트는 "센다 → 통과시킨다 → 나중에 기록한다"이고 그 사이에 잠금이 없다.
질의는 임베딩·검색·생성이, 업로드는 파일 읽기·PDF 파싱·문서 삽입이 그
틈이다. 미인증 계정이 요청을 동시에 던지면 전부 같은 (아직 커밋 안 된)
숫자를 읽고 통과한다. 레이트리밋이 상한을 주므로 무한은 아니다 — 질의는
분당 20, 업로드는 분당 5까지다.

**문서 개수가 아니라 쪽수로 따져야 실제 비용이 보인다.** 처음 구현은
`unverified_quota_documents`(문서 1개)만 있었고 쪽수는 아예 보지 않았다 —
그 상태에서는 이 틈으로 500쪽짜리 PDF 한 장, 심지어 동시 업로드 5개로
**최대 2,500쪽**(인증 계정의 월간 전체 한도 1,000쪽의 2.5배)을 공짜로
임베딩할 수 있었다. `unverified_quota_pages`(50)를 업로드 1건마다 따로
검사하도록 고친 뒤로는 같은 경쟁 조건에서도 각 요청이 개별적으로 50쪽을
넘지 못하므로, 상한은 업로드 레이트리밋(분당 5)과 곱해 **최대 250쪽**이다
— 여전히 원자적이지 않지만 통화가 쪽수로 바뀌었다.

고치지 않은 이유는 **기존 쿼터가 이미 같은 모양**이기 때문이다
(`query_quota_exceeded`·`upload_quota_exceeded`도 check-then-act이다).
미인증 경로만 원자적으로 만들면 같은 파일 안에서 두 규칙이 다른 보증을
갖게 된다. 고친다면 둘 다여야 하고, 그건 이메일 인증이 아니라 쿼터
집행의 문제다. 다만 막으려던 것과 비교하면 아직 낫다 — 재가입은 한도를
**무제한으로** 초기화하지만 이 우회는 한 계정에서 상수배(최대 250쪽)에
그친다.
...
- **동시 소비는 정확히 하나만 성공**하고 나머지는 "이미 인증됨"으로 끝난다
- **재발송과 겹친 소비는 예외가 아니라 거부**로 끝난다 (아래 참조)
